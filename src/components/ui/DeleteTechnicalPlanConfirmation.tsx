...
interface DeleteTechnicalPlanConfirmationProps {
    plan: TechnicalPlan;
    onClose: () => void;
    onSuccess: () => void;
}

export function DeleteTechnicalPlanConfirmation({ plan, onClose, onSuccess }: DeleteTechnicalPlanConfirmationProps) {
    ...
    return (
        <Modal isOpen={true} onClose={onClose} title="Confirmar Eliminación">
            <div className="p-6">
                <div className="mb-6">
                    <h3 className="text-lg font-medium text-gray-900 mb-1">Eliminar Plano Técnico</h3>
                    <p className="text-sm text-gray-500 mb-4">Esta acción no se puede deshacer</p>
                    <div className="bg-gray-50 rounded-lg p-4">
                        <div className="text-sm text-gray-700">
                            <div><span className="font-medium">Título:</span> {plan.title}</div>
