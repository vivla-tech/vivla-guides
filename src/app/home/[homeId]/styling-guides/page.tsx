...
import { Home, Room, StylingGuide, Playbook } from '@/lib/types';
...
export default function StylingGuidesPage() {
    ...
    useEffect(() => {
        const loadHomeData = async () => {
            try {
                ...
                // Cargar habitaciones de la casa
                const roomsResponse = await apiClient.listRooms({ home_id: homeId, pageSize: 100 });
    ...
    // Funciones helper para obtener guías y playbooks por habitación
    const getGuidesForRoom = (roomId: string) => {
        return stylingGuides.filter(guide => guide.room_id === roomId);
    };

    const getPlaybooksForRoom = (roomId: string) => {
        return playbooks.filter(playbook => playbook.room_id === roomId);
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {/* Lista Organizada por Habitaciones */}
                <div className="bg-white rounded-lg shadow">
                    <div className="px-6 py-4 border-b border-gray-200">
                        <h2 className="text-lg font-medium text-gray-900">
                            Guías de Estilismo y Playbooks por Habitación
                        </h2>
                    </div>
                    ...
                    {rooms.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <p className="text-gray-500 mb-4">
                                Primero necesitas crear habitaciones para esta casa.
                            </p>
                            <Link
                                href={`/wizard/styling-guides?homeId=${homeId}`}
                        ...
                        <div className="divide-y divide-gray-200">
                            {rooms.map((room) => {
                                const roomGuides = getGuidesForRoom(room.id);
                                const roomPlaybooks = getPlaybooksForRoom(room.id);
                                ...
                                return (
                                    <div key={room.id} className="p-6">
                                        ...
                                        {/* Guías de Estilo de esta habitación */}
                                        {roomGuides.length > 0 && (
                                            <div className="mb-6">
                                                <h4 className="text-md font-medium text-gray-700 mb-3">
                                                    🎨 Guías de Estilo ({roomGuides.length})
                                                </h4>
                                                        ...
                                                        <div key={guide.id} className="border border-gray-200 rounded-lg p-4">
                                                            <div className="flex items-start space-x-4">
                                                                ...
                                                                <div className="flex-1">
                                                                    ...
                                                                    {guide.image_urls && guide.image_urls.length > 0 && (
                                                                        <div className="mt-3">
                                                                            <h6 className="text-sm font-medium text-gray-700 mb-2">Galería de Inspiración</h6>
                                        ...
                                        {roomGuides.length === 0 && roomPlaybooks.length === 0 && (
                                            <div className="text-center py-6 text-gray-500">
                                                <p>No hay guías de estilo ni playbooks en esta habitación</p>
                                            </div>
                                        )}
                                    </div>
                                );
